...
global_countries = ['usa', 'united states', 'china', 'uk', 'united kingdom', 'germany', 'canada', 'france', 'australia', 'japan', 'south korea', 'india', 'italy', 'spain', 'switzerland', 'netherlands', 'singapore']
fallback_mapping = {
    'USA': r'\b(stanford|mit|google|microsoft|facebook|meta|berkeley|cmu|harvard|apple|amazon|nvidia|ibm)\b',
    'China': r'\b(tsinghua|beijing|tencent|alibaba|baidu|huawei|buaa|sjtu|zju|fudan)\b',
    'UK': r'\b(oxford|cambridge|london|deepmind|ucl)\b',
    'Canada': r'\b(toronto|montreal|mila|waterloo)\b'
}
industry_keywords = [r'\binc\b', r'\bcorp\b', r'\bcompany\b', r'\bgoogle\b', r'\bmicrosoft\b', r'\bmeta\b', r'\btencent\b', r'\balibaba\b', r'\bbaidu\b', r'\bapple\b', r'\bamazon\b', r'\bibm\b']
academic_keywords = [r'\buniv', r'\buniversity\b', r'\binstitute\b', r'\bcollege\b', r'\bschool\b', r'\bacademy\b']

def _get_country(aff):
    if pd.isna(aff): return 'Other'
    aff_lower = str(aff).lower()
    for country, pattern in fallback_mapping.items():
        if re.search(pattern, aff_lower): return country
    for c in global_countries:
        if re.search(r'\b' + c + r'\b', aff_lower):
            if c in ['usa', 'united states']: return 'USA'
            if c in ['uk', 'united kingdom']: return 'UK'
            if c == 'south korea': return 'South Korea'
            return c.title() 
    return 'Other'

def _get_org_type(aff):
    if pd.isna(aff): return 'Other'
    aff_lower = str(aff).lower()
    if any(re.search(kw, aff_lower) for kw in industry_keywords): return 'Industry'
    if any(re.search(kw, aff_lower) for kw in academic_keywords): return 'Academic'
    return 'Other'

def _prep_s3(df):
    df['country'] = df['affiliationName'].apply(_get_country)
    df = df.drop_duplicates(subset=['pub', 'country'])
    return df[df['year'] >= 2000]

def s3q1(df):
    df = _prep_s3(df)
    print("        TOP 10 AI COUNTRIES")
    print(df['country'].value_counts().drop('Other', errors='ignore').head(10))

def s3q2(df):
    df = _prep_s3(df)
    yearly_dist = df.groupby(['year', 'country']).size().unstack(fill_value=0)
    top_cols = df['country'].value_counts().drop('Other', errors='ignore').head(6).index
    yearly_dist[top_cols].plot(kind='line', figsize=(10, 5), linewidth=2)
    plt.title("AI Publications by Year for Top Countries")
    plt.xlabel("Year")
...
def s3q3(df):
    df = _prep_s3(df)
    yearly_dist = df.groupby(['year', 'country']).size().unstack(fill_value=0)
    print("   FASTEST GROWTH (CAGR 2018-2023)")
    if 2018 in yearly_dist.index and 2023 in yearly_dist.index:
        valid_cols = yearly_dist.columns[(yearly_dist.loc[2018] >= 20) & (yearly_dist.columns != 'Other')]
        recent = yearly_dist.loc[2018:2023, valid_cols]
        cagr = (recent.loc[2023] / recent.loc[2018]) ** (1/5) - 1
        print((cagr * 100).sort_values(ascending=False).head(5).apply(lambda x: f"{x:.2f}%"))

def s3q4(df):
    df = _prep_s3(df)
    yearly_dist = df.groupby(['year', 'country']).size().unstack(fill_value=0)
    balance = yearly_dist.div(yearly_dist.sum(axis=1), axis=0) * 100
    plot_cols = [c for c in ['USA', 'China', 'UK', 'Canada','Germany'] if c in balance.columns]
    balance[plot_cols].plot(kind='area', stacked=True, figsize=(10, 5))
    plt.title("Global Balance of AI Research (%)")
    plt.margins(x=0)
    plt.show()

def _prep_s4(df):
    df['year'] = pd.to_numeric(df['year'], errors='coerce')
    return df[df['year'] >= 2000].dropna(subset=['year'])

def s4q1(df):
    df = _prep_s4(df)
    print(f"Total Unique Authors in AI: {df['authorName'].nunique():,}")
...
def s4q3(df):
    df = _prep_s4(df)
    avg_trend = df.groupby(['year', 'pub']).size().reset_index(name='c').groupby('year')['c'].mean()
    avg_trend.plot(kind='line', marker='o', color='teal', figsize=(10, 5))
    plt.title("Average Number of Authors per Publication Over Time")
    plt.show()

def s4q4(df_s3):
    df = _prep_s3(df_s3)
    df_valid = df[df['country'] != 'Other'].dropna(subset=['country'])
    c_per_pub = df_valid.groupby(['year', 'pub'])['country'].nunique().reset_index(name='count')
    (c_per_pub.groupby('year').apply(lambda x: (x['count'] > 1).mean()) * 100).plot(kind='line', marker='s', color='orange', figsize=(10, 5))
    plt.title("Percentage of International Collaborations (%)")
    plt.show()

def s4q5(df_geography):
    print("="*50)
    print(" International Collaborations (Percentage vs. Absolute)")
    print("="*50)
    df_geography['year'] = pd.to_numeric(df_geography['year'], errors='coerce')
    df_valid = df_geography[(df_geography['year'] >= 2000) & (df_geography['country'] != 'Other')].dropna(subset=['country'])
    countries_per_pub = df_valid.groupby(['year', 'pub'])['country'].nunique().reset_index(name='country_count')
    intl_pct = countries_per_pub.groupby('year').apply(lambda x: (x['country_count'] > 1).mean()) * 100
    intl_abs = countries_per_pub.groupby('year').apply(lambda x: (x['country_count'] > 1).sum())
    fig, axes = plt.subplots(1, 2, figsize=(16, 5))
    intl_pct.plot(kind='line', marker='s', color='orange', linewidth=2, ax=axes[0])
    axes[0].set_title("Percentage of International Collaborations (%)", fontsize=14)
    axes[0].set_xlabel("Year")
    axes[0].set_ylabel("% of Papers")
    axes[0].grid(True, linestyle='--', alpha=0.6)
    intl_abs.plot(kind='line', marker='o', color='blue', linewidth=2, ax=axes[1])
    axes[1].set_title("Absolute Number of International Collaborations", fontsize=14)
...
def _prep_s5(df):
    df['year'] = pd.to_numeric(df['year'], errors='coerce')
    df = df[df['year'] >= 2000].dropna(subset=['year'])
    df['org_type'] = df['affiliationName'].apply(_get_org_type)
    return df.drop_duplicates(subset=['pub', 'affiliationName'])

def s5q1(df):
    df = _prep_s5(df)
    print("       TOP 10 INSTITUTIONS")
    print(df['affiliationName'].value_counts().head(10))

def s5q2(df):
    df = _prep_s5(df)
    top_inst = df['affiliationName'].value_counts().head(5).index
    df[df['affiliationName'].isin(top_inst)].groupby(['year', 'affiliationName']).size().unstack(fill_value=0).plot(kind='line', figsize=(10, 5), marker='o')
    plt.title("Contribution of Leading Institutions Over Time")
...
def s5q3(df):
    df = _prep_s5(df)
    top_inst = df['affiliationName'].value_counts().head(5).index
    df_top = df[df['affiliationName'].isin(top_inst)].copy()
    df_top['conf'] = df_top['pub'].str.extract(r'/conf/([^/]+)/', expand=False).str.upper()
    spec = pd.crosstab(df_top['affiliationName'], df_top['conf'])
    sns.heatmap(spec.div(spec.sum(axis=1), axis=0) * 100, annot=True, cmap="YlGnBu", fmt=".1f")
    plt.title("Subtopic Specialization (%)")
    plt.show()

def s5q4(df):
    df = _prep_s5(df)
    trend = df.groupby(['year', 'org_type']).size().unstack(fill_value=0)
    cols = [c for c in ['Academic', 'Industry'] if c in trend.columns]
    trend[cols].plot(kind='line', figsize=(10, 5), linewidth=2.5)
    plt.title("Academic vs. Industrial Dominance")
    plt.show()
